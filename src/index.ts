class MTACard extends HTMLElement {
  ...
  set hass(hass: HomeAssistant) {
    ...
    const trainRowData: TrainRowProps[] = [];
...
window.customCards.push({
  ...
  name: 'MTA Card',
  description:
    'A card that works with the ha-gtfs-rt integration to display MTA train times in the format and appearance of the subway.',
});

function getDueIn(state: SensorProps) {
  return state.attributes['Due in'];
}

function getNextDueIn(state: SensorProps) {
  return state.attributes['Next bus due in'];
}

function getIconLink(lineName: string) {
